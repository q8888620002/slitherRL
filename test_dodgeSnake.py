...
if __name__ == '__main__':
  ...
  for i_episode in range(200):
     ...
     for t in range(1000):
       ...
       action = universe.spaces.PointerEvent(observation_n[0,0,0], observation_n[1,0,0])
       ...
       if done_n:
         ...
         done_time = time.time()
         # write to csv file
         myFile = open('output_greedy.csv', 'a')  
         with myFile: 
          writer = csv.writer(myFile)
          data = [i_episode+1, total_reward, start_time, done_time]
